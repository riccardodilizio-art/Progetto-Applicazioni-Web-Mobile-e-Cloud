...
import type { Dish, DayMenu } from '../types/Menu'
import type { ApiMenuResponse } from '../types/Menu'
import { breakfastItems, lunchItems } from '../data/Menu'
import { apiFetch } from '../lib/apiClient'
import { mapApiMenus } from '../lib/mappers'
...
export default function Menu() {
    ...
    useEffect(() => {
        apiFetch<ApiMenuResponse[]>('/menus')
            .then((data) => setMenus(mapApiMenus(data)))
            .catch(console.error)
        ...
        <div className="min-h-screen bg-[#FAF0E6]">
            ...
            <section className="max-w-6xl mx-auto px-4 py-16">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
                    <div className="bg-white rounded-2xl shadow-sm border border-[#E8C9A0] p-6 text-center">
                        <div className="w-12 h-12 bg-[#E8C9A0] rounded-full flex items-center justify-center mx-auto mb-4">
                            <svg aria-hidden="true" className="w-6 h-6 text-[#6B4828]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {loading ? (
                    ...
                    <p className="text-center text-[#9A6840] py-12">Menu non ancora disponibile.</p>
                ) : (
                    <>
                        {/* Selettore giorno */}
                        ...
                        <div className="space-y-10">
                            ...
                            <div className="bg-white rounded-2xl shadow-sm border border-[#E8C9A0] overflow-hidden">
                                <div className="bg-[#FAF0E6] border-b border-[#E8C9A0] px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                    ...
                                    <span className="self-start sm:self-auto bg-[#E8C9A0] text-[#6B4828] text-xs font-medium px-3 py-1 rounded-full whitespace-nowrap">Inclusa nel soggiorno</span>
                                </div>
                                <div className="p-6">
                            ...
                            <div className="bg-white rounded-2xl shadow-sm border border-[#E8C9A0] overflow-hidden">
                                <div className="bg-[#FAF0E6] border-b border-[#E8C9A0] px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                    ...
                                    <span className="self-start sm:self-auto bg-[#E8C9A0] text-[#6B4828] text-xs font-medium px-3 py-1 rounded-full whitespace-nowrap">Incluso nel soggiorno</span>
                                </div>
                                <div className="p-6">
                                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        {lunchItems.map((item) => (
