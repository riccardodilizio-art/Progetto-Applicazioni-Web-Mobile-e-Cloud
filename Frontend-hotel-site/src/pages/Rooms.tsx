            ...
            <div className="px-4">
                <div className="bg-[#FAF5EE] border border-[#C4A070] rounded-2xl shadow-md p-6 max-w-4xl mx-auto -mt-8 relative z-10">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            <label htmlFor="rooms-checkin" className="block text-xs font-semibold text-[#3B2010] uppercase tracking-wide mb-1">Check-in</label>
                            <input id="rooms-checkin" type="date" min={today} value={checkIn} onChange={(e) => handleCheckInChange(e.target.value)} className="w-full border border-[#C4A070] rounded-lg px-3 py-2 text-sm text-[#3B2010] focus:outline-none focus:ring-2 focus:ring-[#9A6840] bg-white" />
                        </div>
                        <div>
                            <label htmlFor="rooms-checkout" className="block text-xs font-semibold text-[#3B2010] uppercase tracking-wide mb-1">Check-out</label>
                            <input id="rooms-checkout" type="date" min={minCheckOut} value={checkOut} onChange={(e) => setCheckOut(e.target.value)} disabled={!checkIn} className="w-full border border-[#C4A070] rounded-lg px-3 py-2 text-sm text-[#3B2010] focus:outline-none focus:ring-2 focus:ring-[#9A6840] bg-white disabled:opacity-50 disabled:cursor-not-allowed" />
                        </div>
                        <div>
                            <label htmlFor="rooms-guests" className="block text-xs font-semibold text-[#3B2010] uppercase tracking-wide mb-1">Ospiti</label>
                            <select id="rooms-guests" value={guests} onChange={(e) => setGuests(Number(e.target.value))} className="w-full border border-[#C4A070] rounded-lg px-3 py-2 text-sm text-[#3B2010] focus:outline-none focus:ring-2 focus:ring-[#9A6840] bg-white cursor-pointer">
                                {[1, 2, 3, 4].map((n) => (
                    ...
                    {(checkIn || checkOut || guests > 1) && (
                        <div className="mt-4 flex items-center justify-between flex-wrap gap-2">
                            ...
                            <button onClick={resetSearch} className="text-sm text-[#9A6840] hover:text-[#6B4828] underline cursor-pointer ml-auto">
                                Cancella filtri
                            </button>
                        </div>
                    )}
