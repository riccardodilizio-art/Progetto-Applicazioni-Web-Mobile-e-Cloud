...
export default function RoomDetail() {
    ...
    useEffect(() => {
        ...
        apiFetch<ApiRoom>(`/rooms/${id}`)
            .then((data) => setRoom(mapApiRoom(data)))
            .catch(() => setRoom(null))
                ...
                {hasSearch && (
                    <div className="bg-[#FAF5EE] border border-[#E8C9A0] rounded-xl px-4 py-3 mb-6 flex items-center justify-between flex-wrap gap-2">
                        <p className="text-sm text-[#6B4828]">
                            <span className="font-semibold">Soggiorno selezionato:</span> {formatDate(checkIn)} → {formatDate(checkOut)} · {guests} {guests === 1 ? 'ospite' : 'ospiti'} · {nights} {nights === 1 ? 'notte' : 'notti'}
                        </p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    ...
                    <div>
                        <div className="rounded-2xl overflow-hidden shadow-lg mb-3">
                            <img src={room.images[currentImage]} alt={`${room.name} - foto ${currentImage + 1}`} className="w-full h-80 md:h-96 object-cover" />
                        </div>
                        {room.images.length > 1 && (
                            <div className="flex gap-3">
                                {room.images.map((img, i) => (
                                    <button key={img} onClick={() => setCurrentImage(i)} className={`rounded-lg overflow-hidden border-2 transition ${currentImage === i ? 'border-[#6B4828]' : 'border-transparent opacity-60 hover:opacity-100'}`}>
                                        <img loading="lazy" src={img} alt={`Miniatura ${i + 1}`} className="w-20 h-14 object-cover" />
                                    </button>
                                ))}
                            </div>
                        )}
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <span className={`text-xs font-semibold px-3 py-1 rounded-full ${room.available ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
